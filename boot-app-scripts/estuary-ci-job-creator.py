...
def create_jobs(base_url, kernel, plans, platform_list, targets, priority,
    ...
    tree = build_info[1] #open-estuary
    kernel_version = build_info[2] #uefi_b13bead_grub_daac831_kernel_1c364c9
    defconfig = build_info[3] #d02-arm64??
    has_modules = True
    checked_modules = False

    pubkey = get_pubkey() #pubkey
    for platform in platform_list:
        platform_name = platform.split('/')[-1] #hip06-d03.dtb
        for device in device_map[platform_name]: #[d03]
            ...
            for plan in plans:
                if 'boot' in plan or 'BOOT' in plan:
                    ...
                    try:
                        config.read(cwd + '/templates/' + plan + '/' + plan + '.ini') #/home/guanhe/ci-scripts/boot-app-scripts/templates/boot/boot.ini
                ...
                if 'BIG_ENDIAN' in defconfig and plan != 'boot-be':
                    print 'BIG_ENDIAN is not supported on %s. Skipping JSON creation' % device_type #d03
                    ...
                    print '%s has been blacklisted. Skipping JSON creation' % defconfig #has been blacklisted
                elif any([x for x in device['kernel_blacklist'] if x in kernel_version]): #x = []
                    print '%s has been blacklisted. Skipping JSON creation' % kernel_version	
                elif any([x for x in device['nfs_blacklist'] if x in kernel_version]) \ #x= []
                    ...
                    if test_kind != "BOTH":
                        ...
                        try:
                            single_templates = [ x for x in device_templates if \
                                    x.split(".json")[0] in \
                                    config_plan.get("TEST_KIND", test_kind).split(",")] #boot.ini
                        except:
...
def walk_url(url, distro_url, plans=None, arch=None, targets=None,
            priority=None, distro="Ubuntu", SasFlag=False): #url = http://114.119.4.74:8083/open-estuary/uefi_b13bead_grub_daac831_kernel_1c364c9/d02-arm64/
    global base_url
    global kernel
    ...
    for name in files:
        if name.endswith('/'):
            dirs += [name]
        if arch is None:
            if 'bzImage' in name and 'x86' in url:  #get kernel url and base url
                kernel = url + name
                base_url = url
            ...
            if 'Image' in name and 'arm64' in url:
                kernel = url + name #http://114.119.4.74:8083/open-estuary/uefi_b13bead_grub_daac831_kernel_1c364c9/arm64/Image
                base_url = url #http://114.119.4.74:8083/open-estuary/uefi_b13bead_grub_daac831_kernel_1c364c9/arm64/
...
def main(args): #python estuary-ci-job-creator.py $FTP_SERVER/${TREE_NAME}/${GIT_DESCRIBE}/${PLAT}-${board_arch}/ --plans $test_name --distro $distro $harddisk_flag
    global test_kind 
