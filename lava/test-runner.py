...
class TestPlan(object):
    ...
    def test_list(self, kind="automated"):
        if self.test_def:
            ...
            try:
                test_list = []
                for requirement in test_plan['requirements']: #test_plan=[metadata:{},requirements:{}] #test_plan['requirements']=[name:{},owner:{},tests:{},...]
                    if 'tests' in requirement.keys():
                        if requirement['tests'] and \
                                kind in requirement['tests'].keys() and \
                                requirement['tests'][kind]:#requirement['tests']['manual'],requirement['tests']['automated']
                            for test in requirement['tests'][kind]:#requirement['tests']['automated'] = [path: manual/linux/test1.yaml],requirement['tests']['manual']=[]
...
class TestSetup(object):
    ...
    def validate_env(self):
        ...
        try:
            self.repo_path = os.environ['REPO_PATH'] #获取环境变量
        except KeyError:
    ...
    def copy_test_repo(self):
        self.validate_env()
        shutil.rmtree(self.test_path, ignore_errors=True) #test_path = output+uuid
        if self.test_kind == 'manual':
            test_dir_path = os.path.join(self.repo_path, self.repo_test_path.rsplit("/", 1)[0]) #REPO_PATH + automated/linux/smoke/
            shutil.copytree(test_dir_path, self.test_path, symlinks=True)
            self.logger.info('Test copied to: %s' % self.test_path)
        else:
            if self.repo_path in self.test_path:#test_path = output+uuid
...
class TestDefinition(object):#把yaml文件里的测试，传给Object
    """
    ...
    def __init__(self, test, args):
        ...
        self.test_name = os.path.splitext(self.test_def.split('/')[-1])[0] #smoke
        ...
        if 'skip_install' in test:
            self.skip_install = test['skip_install'] #kselftest.yaml
        ...
        if os.path.isfile(self.test_def):
            self.exists = True
            with open(self.test_def, 'r') as f:#test_def = $path
                self.testdef = yaml.safe_load(f) #self.testdef = yaml to object
                if self.testdef['metadata']['format'].startswith("Manual Test Definition"):#testdef = ./manual/generic/linux/pxe-boot.yaml
                    self.is_manual = True

    def definition(self):
        with open('%s/testdef.yaml' % self.test_path, 'w') as f: #test_path = $output #test_path=output+uuid
    ...
    def run(self):
        if not self.is_manual:
            with open('%s/run.sh' % self.test_path, 'a') as f:
                ...
                steps = self.testdef['run'].get('steps', []) #usb-storege.yaml
                if steps:
                    for cmd in steps:
    ...
    def handle_parameters(self):#ret_val value_name = value
...
class AutomatedTestRun(TestRun):
    ...
    def check_result(self):
        if self.test_timeout:
            ...
            test_end = time.time() + self.test_timeout  ##>>test_end 预期结束时间
...
class ResultParser(object):
    def __init__(self, test, args):
        ...
        self.results['params'] = {}
        with open(os.path.join(self.result_path, "testdef.yaml"), "r") as f:#output_uuid/testdef.yaml
            self.testdef = yaml.safe_load(f)
            if 'params' in self.testdef.keys():##>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>?test 的唯一性怎么确定
        ...
        with open('%s/stdout.log' % self.result_path, 'r') as f:
            for line in f:
                if re.match(r'\<(|LAVA_SIGNAL_TESTCASE )TEST_CASE_ID=.*', line):
                    ...
                    for string in line:#line is array
                        parts = string.split('=')#part is array
                        if len(parts) == 2:
                            key, value = parts
                            key = key.lower()
                            data[key] = value #
    ...
    def dict_to_json(self):
        ...
        with open('%s/result.json' % self.result_path, 'w') as f:
            json.dump([self.results], f, indent=4)#json format
        ...
        feeds.append(self.results) #>>>why to do it
...
def main():
    # Setup logger.
    logger = logging.getLogger('RUNNER') #传递RUNNER进去，返回logger登陆器
    ...
    test_list = test_plan.test_list(args.kind) #test_list = ["automated":[],"manual":[]]
